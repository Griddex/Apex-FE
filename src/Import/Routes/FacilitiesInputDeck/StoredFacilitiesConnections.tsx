...
export default function StoredFacilitiesConnections({
  reducer,
  containerStyle,
  showChart,
}: IStoredDeck) {
  ...
  const tableTitle = "Facilities Connections";
  const mainUrl = `${getBaseForecastUrl()}/facilities-connections`;
  ...
  const facilitiesConnectionsStoredSelector = createDeepEqualSelector(
    (state: RootState) => state[reducer][wc]["facilitiesConnectionsStored"],
    (stored) => stored
  );
  const facilitiesConnectionsStored = useSelector(
    facilitiesConnectionsStoredSelector
  );
  ...
  const snStoredData =
    facilitiesConnectionsStored &&
    facilitiesConnectionsStored.map(
