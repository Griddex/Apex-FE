...
export default function StoredForecastConnections({
  reducer,
  containerStyle,
  showChart,
}: IStoredDeck) {
  ...
  const tableTitle = "Forecast Connections";
  const mainUrl = `${getBaseForecastUrl()}/forecast-connections`;
  const collectionName = "InputDeckEntities";

  const dispatch = useDispatch();
  const wc = "storedDataWorkflows";
  const wp: NonNullable<IStoredDataProps["wkPs"]> = "forecastConnectionsStored";

  const forecastConnectionsStoredSelector = createDeepEqualSelector(
    (state: RootState) => state[reducer][wc]["forecastConnectionsStored"],
    (stored) => stored
  );
  const forecastConnectionsStored = useSelector(
    forecastConnectionsStoredSelector
  );
