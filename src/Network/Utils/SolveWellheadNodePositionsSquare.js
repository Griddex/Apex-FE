...
const SolveWellheadNodePositionsSquare = (
  manifoldPosition,
  numberOfWells,
  wellheadGroupOffset
) => {
  ...
  const wellheadPositions = realdiffs.map((diff) => {
    return {
      x: diff.a + manifoldPosition.x,
      y: diff.b + manifoldPosition.y + wellheadGroupOffset,
    };
  });

  return wellheadPositions;
};
export default SolveWellheadNodePositionsSquare;
